...
- Soft multilabel-guided discriminative embedding learning (MDL)
  - SM-guided hard negative mining: unlabeled target 도메인에 대한 분별력있는 embedding feature $$f(\cdot)$$을 학습하는 방식. Unlabeled target pair $$(x_i, x_j)$$에서 시각적 특징 $$f(x_i)^T f(x_j)$$과 soft multilabel agreement $$A(y_i, y_j)$$을 보고 similarity consistency를 분석. (시각적으로 서로 유사하지만 $$f(x_i)^T f(x_j)>=S$$ SM agreement이라는 다른 관점이 서로 다르다면 $$A(y_i, y_j)<T$$ hard negative로 판단. 둘다 비슷하면 positive set) 한 batch 내에서 총 가능한 pair 수를 다음과 같이 표현할 때 $$M=N_{batch} \times (N_{batch} - 1 )/2$$ ($$N_{batch}$$는 한 batch 내에서 unlabeled target image의 수), mining ratio $$p$$를 곱하여 시각적으로 유사한 $$pM$$ pair 중에서 Positive set와 negative set의 비율은 threshold $$T$$를 이용하여 조정. 
