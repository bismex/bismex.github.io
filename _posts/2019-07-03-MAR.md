...
| $$\mathcal{Z}=\left \{ z_i, w_i \right \}^{N_a}_{i=1}$$ | labeled auxiliary (reference) REID dataset (MSMT17, #ID-4101, #Img-126441) |
| $$z_i$$ | reference image |
...
- Soft multilabel-guided discriminative embedding learning (MDL)
  - SM-guided hard negative mining: unlabeled target 도메인에 대한 분별력있는 embedding feature $$f(\cdot)$$을 학습하는 방식. Unlabeled target pair $$(x_i, x_j)$$에서 시각적 특징 $$f(x_i)^T f(x_j)$$과 soft multilabel agreement $$A(y_i, y_j)=1-\frac{\left \| y_i - y_j \right \|_1}{2}$$을 보고 similarity consistency를 분석. 시각적으로 서로 유사하지만 (i.e., $$f(x_i)^T f(x_j)>=S$$) SM agreement이라는 다른 관점이 서로 다르다면 (i.e., $$A(y_i, y_j)<T$$) hard negative로 판단하고 두 관점에서 한 쌍의 영상이 비슷하면 positive set으로 판단. 한 batch (=368) 내에서 총 가능한 pair 수를 다음과 같이 표현할 때 $$M=N_{batch} \times (i.e., N_{batch} - 1 )/2$$ ($$N_{batch}$$는 한 batch 내에서 unlabeled target image의 수, 한 batch는 무작위로 샘플링한 $$x, z$$를 절반씩 구성), mining ratio $$p$$를 곱하여 시각적으로 유사한 $$pM$$ pair 중에서 Positive set와 negative set의 비율은 threshold $$T$$를 이용하여 조정. 여기에서 hypersphere leaning을 위해 $$\left \| f(\cdot) \right \|_2=1$$, $$\left \| a_i \right \|_2=1$$라고 가정 (하지만 이는 수렴 문제 때문에 초기에는 unit norm constraint 없이 $$L_AL$$만 가지고 network 학습한다.).
