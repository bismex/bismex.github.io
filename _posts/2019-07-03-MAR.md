...
- Reference agent learning (RAL)
  ...
  - Reference agent joint embedding learning loss (RJ)
    - 학습대상
    - Loss function
      - $$L_{RJ} = \sum_i \sum_{j \in \mathcal{M}_i} \sum_{k \in \mathkal{W}_i} \left \[ m - \reft \| a_i - f(x_j) \right \|^2_2 \right \]_+ + \eft \| a_i - f(z_k) \right \|^2_2 $$
         - $$\mathcal{M}_i = {j| \left \| a_i - f(x_j)\right \|^2_2 < m}$$, $$m=1$$
         - $$\mathcal{W}_i = {k|w_k = i}$$
         - $$\left \| a_i - f(z_k) \right \|^2_2$$
