  ...
  - Reference agent joint embedding learning loss (RJ)
    ...
    - Loss function
      - $$L_{RJ} = \sum_i \sum_{j \in \mathcal{M}_i} \sum_{k \in \mathcal{W}_i} (1) + (2) $$
         - $$(1) \left [ m - \left \| a_i - f(x_j) \right \|^2_2 \right ]_+$$: 서로 비슷한 cross-domain pair가 마진만큼 멀어지도록 (분별력)
           - Agent-based margin $$m=1$$, $$\left [ \quad \right ]_+$$: hinge loss (margin에 가까워질수록 loss가 감소하는, margin보다 크면 0)
           - $$\mathcal{M}_i = \left \{ j | \left \| a_i - f(x_j) \right \|^2_2 < m \right \}$$, $$m=1$$: $$j$$는 cross-domain pair 차이가 margin보다 작은 후보들 (비슷한 후보들)
         - $$(2) \left \| a_i - f(z_k) \right \|^2_2$$: reference agent와 해당 label에 대응하는 reference images를 최대한 가깝게 (대표성)
    ...
    - 서로 다른 도메인 사이의 분포의 정렬을 위해 cross-domain hard negative pair $$\mathcal{M}_i$$를 수집함 (i.e., 시각적으로 비슷한)
