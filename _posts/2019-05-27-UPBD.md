...
*Notation: spatial image transformation $$s$$, image transformation $$a(x)$$, part-based factorization $$\phi(x) := (\phi_1 (x), \phi_2 (x), ... )^T$$, disentangled representation $$\phi_i (x) = (\alpha_i (x), \sigma_i (x))$$, part appearance $$\alpha_i (x)$$, part activation map $$\sigma_i (x)$$, decoder $$\mathit{D}$$, shape hourglass network $$\mathit{E}_\sigma$$, appearance hourglass network $$\mathit{E}_\alpha$$*
