...
<br/><p align="center" style="color: #e01f1f; font-weight: bold;">[전체 프레임워크]</p>
![picture]({{ '/assets/images/MAR02.png' | relative_url }}){: style="width: 100%;" class="center"}<br/>
...
- Reference agent learning (RAL)
  ...
  - Reference agent joint embedding learning loss (RJ)
    - 학습대상 $$f(\cdot), \left \{ a_i \right \}^{N_p}_{i=1}$$
    - Loss function
      - $$L_{RJ} = \sum_i \sum_{j \in \mathcal{M}_i} \sum_{k \in \mathcal{W}_i} (1) + (2) $$
        - $$(1) \left [ m - \left \| a_i - f(x_j) \right \|^2_2 \right ]_+$$
          ...
          - $$\mathcal{M}_i = \left \{ j | \left \| a_i - f(x_j) \right \|^2_2 < m \right \}$$: $$j$$는 cross-domain pair 차이가 margin보다 작은 후보들 (비슷한 후보들)
          - $$\left [ \quad \right ]_+$$: hinge loss (margin에 가까워질수록 loss가 감소하는, margin보다 크면 0)
          - $$m=1$$: Agent-based margin 
        - $$(2) \left \| a_i - f(z_k) \right \|^2_2$$
          - Reference agent와 해당 label에 대응하는 reference images를 최대한 가깝게 (대표성)
          - $$\mathcal{W}_i = \left \{ k | w_k = i \right \}$$: $$k$$는 해당하는 $$i$$번째 reference agent에 대응하는 reference label index
...
- 기타 실험
  ...
  - Hyperparameter 평가
    - $$L_{MAR} = L_{MDL} + \lambda_1 L_{CML} + \lambda_2 L_{RAL}$$에서 $$\lambda_1 = 0.0002$$, $$\lambda_2 = 50$$이 가장 성능이 좋고 비교적 넓은 범위에서 성능 안정
  - 기타 파라미터들
    - Mining ratio $$p=5%$$, $$L_{RAL} = L_{AL} + \beta L_{RJ}$$에서 $$\beta=0.2$$이며 이는 supp에서 추가 고찰
