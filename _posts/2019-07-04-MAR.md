...
- Reference agent learning (RAL)
  ...
  - Reference agent joint embedding learning loss (RJ)
    ...
    - (1)설명
      ...
      - $$\mathcal{M}_i = \left \{ j | \left \| a_i - f(x_j) \right \|^2_2 < m \right \}$$에서 $$j$$는 cross-domain pair 차이가 margin보다 작은 후보들 (비슷한 후보들)
    ...
    - (2)설명
      ...
      - $$\mathcal{W}_i = \left \{ k | w_k = i \right \}$$에서 $$k$$는 해당하는 $$i$$번째 reference agent에 대응하는 reference label index
