...
- Loss function
  ...
  - Appearance generative network
    ...
    - ***Pose loss***
      - 사용: $$(\tilde{I}_{P_s}, P_s)$$, pose detector $$\mathit{P}$$, $$(\tilde{I}_{P_t}, P_t)$$
      - Loss: 
      - [6]에서처럼 생성된 영상들 $$\tilde{I}_{P_s}$$, $$\tilde{I}_{P_t}$$ 에 pose detector에 통과한 결과가 $$P_s$$ $$P_t$$와 비슷한지 확인
    - ***Content loss***
      - 사용: $$(I_{P_s}, \tilde{I}_{P_s})$$, VGG feature map
      - Loss: 
      - Source image의 cycle consistency를 보증하기 위해 ImageNet에서 학습한 VGG16 모델의 conv2_1 layer를 통해 복원된 영상과의 텍스처 사이를 확인
    - ***Sytle loss***
      - 사용: $$(I_{P_s}, \tilde{I}_{P_t}, S_{P_s}, \tilde{S}_{P_t})$$, $$(\tilde{I}_{P_s}, \tilde{I}_{P_t}, \tilde{S}_{P_s}, \tilde{S}_{P_t})$$, gram matrix, down-sampling
      - Loss: 
      ...
      - 
    - ***Face loss***
      - 사용: $$(I_{P_s}, \tilde{I}_{P_t})$$, $$(I_{P_s}, \tilde{I}_{P_s})$$, face extraction $$\mathit{F}$$
      - Loss: $$L^{face}_A = L^{adv} (G_A \circ (E_A, E'_S), D_F, \mathit{F}(I_{P_s}), \mathit{F}(\tilde{I}_{P_t})) + L^{adv} (G_A \circ (E_A, E'_S), D_F, \mathit{F}(I_{P_s}), \mathit{F}(\tilde{I}_{P_s}))$$
      - $$D_S$$가 생성된 semantic map이 실제인지 아닌지 구분 
